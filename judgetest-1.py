...
p_in_8= Pin('P8', Pin.IN, Pin.PULL_DOWN)#设置p_in为输入引脚，并开启上拉电阻  Pin.PULL_DOWN 输入下拉电阻 判断是否开始分球
p_in_9= Pin('P9', Pin.IN, Pin.PULL_DOWN)#设置p_in为输入引脚，并开启上拉电阻  Pin.PULL_DOWN 输入下拉电阻 判断是否开始分球判断射球是否就绪执行分球
...
def find_color():
    img = sensor.snapshot()
    for c in img.find_circles(threshold = 1000, x_margin = 30, y_margin = 50, r_margin = 5,
        ...
        if 60<statistics.l_mode()<80 and 50<statistics.a_mode()<80 and -50<statistics.b_mode()<-30:#if the circle is pink
            ...
            p=p+1         #p:检测到的粉球数量
            t=t+1
        elif 80<statistics.l_mode()<101 and -15<statistics.a_mode()<25 and -15<statistics.b_mode()<5:#if the circle is white
            ...
            b=b+1         #b:检测到的黑球数量
            t=t+1
        elif 20<statistics.l_mode()<60 and -15<statistics.a_mode()<25 and -15<statistics.b_mode()<5:#no ball
            no_ball=no_ball+1
        if color_flag==1 and t>10:#抽签为白色 且检测颜色超过10次
